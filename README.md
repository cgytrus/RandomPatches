# ConfiG's Random Patches

A collection of small tweaks and patches designed primarily for my own use or experimental things
that are not stable or polished enough to become separate mods yet.

Beware that anything here may change or get removed at any time and some things might be unstable
or outright broken or disabled. You're welcome to report bugs,
but I cannot guarantee they will be fixed.

Everything is tweaked to myself by default so make sure to check the settings!

## Included mods/patches
not all of them are configurable

some of these are bugfixes that i pr to misc bugfixes
but since it takes a while for cvolton to review and merge the prs
and release an update i put them here temporarily

#### Flipped Particles Fix (broken, removed for now)
Fixes some particles not being flipped correctly in reverse gravity

#### Global Ship Drag Particles (broken, removed for now)
Makes ship drag particles global

#### Wave Trail on Death
i added this before we had megahack v8

#### Custom Percent Decimal
Change the amount of decimal places displayed with decimal percent on

#### Swing Default Mini Icon Fix (in misc bugfixes now but im keeping it)
Fixes the swing icon getting replaced by the cube icon
when switching to mini on swing with default mini icon on

#### Logarithmic Volume Sliders
Puts the volume sliders on a logarithmic scale to more accurately reflect perceived loudness.
Also removes the 0.03 volume snap

#### Icon Explosion Without Explosion
Removes any forces from the icon explosion effect so it just falls apart on death

#### CCDrawNode Draw Area Fix (misc bugfixes pending)
Fixes uninitialized bool causing drawnodes to straight up stop working sometimes
(you mightve seen this manifest in better editor trails or better easing preview)

#### Dash Scale Fix (misc bugfixes pending)
Fixes cube scale remaining at 0.9 when starting dashing as cube or ball
and stopping dashing as robot then switching back to cube

#### Options Keybind (non-iOS)
Adds a Custom Keybinds keybind that opens the vanilla options menu

### Windows Only
i didnt care enough to implement these on other platforms

#### Background on Unfocus (removed rn)
i dont remember what this was so i didnt update it to 2.2074 but if u know what it was for lmk lmao
